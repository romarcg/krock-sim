 ...
 * rosrun gmapping slam_gmapping scan:=/pioneer3at/lms291/laser_scan/layer0 _xmax:=30 _xmin:=-30 _ymax:=30 _ymin:=-30 _delta:=0.2
 */
...
void updateSpeed() {
  ...
  if (obstacle > OBSTACLE_THRESHOLD){
    ...
    speeds[0] = CRUISING_SPEED;
    speeds[1] = CRUISING_SPEED;
    speeds[2] = CRUISING_SPEED;
    speeds[3] = CRUISING_SPEED;
  }
  // set speeds
...
void broadcastTransform() {
  ...
  br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "base_link", "pioneer3at/lms291"));
}
...
int main(int argc, char **argv) {
  ...
  set_lidar_client = n->serviceClient<webots_ros::set_int>("pioneer3at/lms291/enable");
  lidar_srv.request.value = TIME_STEP;
  if (set_lidar_client.call(lidar_srv) && lidar_srv.response.success) {
    ROS_INFO("Lidar enabled.");
    sub_lidar_scan = n->subscribe("pioneer3at/lms291/laser_scan/layer0", 10, lidarCallback);
  ...
  ROS_INFO("You can now start the creation of the map using 'rosrun gmapping slam_gmapping scan:=/pioneer3at/lms291/laser_scan/layer0 _xmax:=30 _xmin:=-30 _ymax:=30 _ymin:=-30 _delta:=0.2'.");
